...
-- Check if houses table exists and has the expected structure
-- If not, create it with integer ID (matching your existing structure)
...
-- Create posts table with integer house_id to match existing houses table
CREATE TABLE IF NOT EXISTS posts (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    house_id INTEGER REFERENCES houses(id),  -- Changed from UUID to INTEGER
