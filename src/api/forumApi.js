    ...
    if (!profile && userId) {
        try {
            ...
            if (user && user.id === userId) {
                ...
                const resolvedName = nameCandidates.find(name => String(name).trim().length > 0);
                
                return {
                    id: user.id,
                    wizard_name: resolvedName ? String(resolvedName).trim() : 'Usuario',
                    avatar_url: userMetadata.avatar_url || null
                };
            }
        } catch (error) {
            console.warn('Error obteniendo datos del usuario actual:', error);
        }
    }
    
    if (!profile) {
        return { wizard_name: 'Usuario Anónimo', avatar_url: null };
    }
...
class ForumAPI {
    ...
    async enrichPostWithRelations(post) {
        try {
            ...
            return {
                ...post,
                user: await normalizeUserProfile(userResult.data, post.user_id),
            ...
            return {
                ...post,
                user: await normalizeUserProfile(null, post.user_id),
                house: null,
                category: null,
                likes: [{ count: 0 }],
                comments: [{ count: 0 }]
    ...
    async enrichPostsWithRelations(posts) {
        try {
            ...
            for (const u of (usersResult.data || [])) {
                usersMap.set(u.id, await normalizeUserProfile(u, u.id));
            }
            ...
            for (const post of posts) {
                let user = usersMap.get(post.user_id);
                if (!user) {
                    user = await normalizeUserProfile(null, post.user_id);
                }
    ...
    async getComments(postId, page = 1) {
        try {
            ...
            for (const c of comments) {
                let user = usersMap.get(c.user_id);
                if (!user) {
                    user = await normalizeUserProfile(null, c.user_id);
                }
